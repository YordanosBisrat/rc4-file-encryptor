...
// ---- RC4 PRGA + XOR (used for both encryption and decryption) ----
void rc4_crypt(unsigned char *S, unsigned char *data, long data_len) {
    ...
    for (long n = 0; n < data_len; n++) {
        ...
        data[n] ^= k;   // XOR with keystream byte
    }
}
...
int main(void) {
    ...
    if (fp == NULL) {
        printf("Error: Could not open '%s'. Make sure it exists in this folder.\n", filename);
        return 1;
    }
    ...
    // Writing encrypted bytes back to the same file
    ...
    printf("Encryption complete. %ld bytes written to %s\n", file_size, filename);
    return 0;
}